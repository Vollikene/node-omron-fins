            ...
            if (SID >= options.minSID && SID <= options.maxSID) {
                ...
                seq.timer = setTimeout(function () {
                    ...
                    if (seq.callback) {
                        ...
                        throw new Error("Sequence Timeout");
                    }
                }, options.timeoutMS);
                return seq;
            }
...
FinsClient.prototype.read = function (address, regsToRead, callback) {
    ...
    if(!regsToRead){
        self.emit('error', new Error("regsToRead is empty"));
        return "";
    }
...
FinsClient.prototype.write = function (address, dataToBeWritten, callback) {
    ...
    if(!dataToBeWritten){
        self.emit('error', new Error("dataToBeWritten is empty"));
        return "";
    }
