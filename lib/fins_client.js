...
FinsClient.prototype.init = function (port, host, options) {
    ...
    self._socket_handler_connect = socket_connect.bind(self);
...
FinsClient.prototype.connect = function (host, port, options) {
    ...
    case 'udp':
        ...
        self.socket.on('connect', self._socket_handler_connect);
    ...
    case 'tcp':
        ...
        self.tcp_socket.on('connect', self._socket_handler_connect);
        break;
    default:
...
function socket_connect() {
    /** @type {FinsClient}*/ const self = this;
    self.connected = true;
    self.emit('connect', self.remoteInfo);
}
...
function tcp_socket_init_listening(err, data) {
    ...
    */
    const client_node = isInt(self.options.client_node, 0);
    ...
    fins_tcp_header.writeInt32BE(0x0C, 4); /* Length */
    fins_tcp_header.writeInt32BE(0x00, 8); /* Command */
    fins_tcp_header.writeInt32BE(0x00, 12); /* Error Code */
    fins_tcp_header.writeInt32BE(client_node, 16);  /* Client Node Add */
