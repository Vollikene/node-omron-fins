...
FinsClient.prototype.init = function (port, host, options) {
    ...
    function receive(buf, rinfo) {
        ...
        const response = _processReply(buf, rinfo, self.sequenceManager);
        if (response) {
...
FinsClient.prototype.read = function (address, count, opts, tag) {
    ...
    const packet = buildPacket(header, command, addressData, wordsToBytes(count));
...
FinsClient.prototype.write = function (address, data, opts, tag) {
    ...
    const packet = buildPacket(header, command, addressData, regsToWrite, dataBytesToWrite);
...
FinsClient.prototype.fill = function (address, value, count, opts, tag) {
    ...
    const packet = buildPacket(header, command, addressData, wordsToBytes(count), dataBytesToWrite);
...
FinsClient.prototype.transfer = function (srcAddress, dstAddress, count, opts, tag) {
    ...
    var packet = buildPacket(header, command, commandData);
